  ...
  var drJs = {
    ...
    swiperActivation: function (e) {
      ...
      $(document).ready(function () {
        ...
        const swiper = new Swiper('.mySwiper-banner-2', {
          effect: 'fade',
          loop: true,
          autoplay: {
            delay: 4000,
      ...
      $(document).ready(function () {
        // Initialize Swiper and store it in a variable
        ...
        // On click of custom "up" button → slidePrev
        $('.carousel-up').on('click', function () {
          mySwiper.slidePrev();
        });

        // On click of custom "down" button → slideNext
    ...
    progressAvtivation: function () {
      ...
      $(window).scroll(function () {
        if (!progressTriggered && $(this).scrollTop() > 250) {
    ...
    smoothScroll: function (e) {
      // Smooth scroll for all anchor links (not just .onepage)
      $(document).on('click', 'a[href^="#"]', function (event) {
        var target = $(this).attr('href');
        if (target && target !== '#' && target.length > 1 && $(target).length) {
          event.preventDefault();
    ...
    preloader: function () {
      ...
      if (preload.length) {
        ...
        // Safety: force-hide preloader after 4 seconds max (prevents stuck loader)
        setTimeout(function () {
...
$('#mobileToggle').on('click', function (e) {
  ...
  // If closing nav, also close all open dropdowns
  if ($nav.hasClass('none')) {
    $nav.find('.dropdown-menu').removeClass('toggle-menu');
  }
});
...
$('.nav-item > .dropdown-toggle').on('click', function (e) {
  ...
  if ($(window).width() <= 1025) {
    ...
    $('.nav-item .dropdown-menu')
      .not($(this).next())
      .removeClass('toggle-menu');
    $(this).next('.dropdown-menu').toggleClass('toggle-menu');

    // Rotate arrow icon
    $('.nav-item > .dropdown-toggle').not(this).removeClass('menu-open');
    $(this).toggleClass('menu-open');
  }
});

// ===== SUBMENU TOGGLE (Mobile click) =====
$('.dropdown-submenu > .dropdown-toggle, .dropdown-submenu > a').on('click', function (e) {
  if ($(window).width() <= 1025) {
    ...
    if ($submenu.length) {
      ...
      $(this)
        ...
        .not($submenu)
        .removeClass('toggle-menu');
      $submenu.toggleClass('toggle-menu');

      // Rotate arrow
      $(this).closest('.dropdown-menu')
        .find('.dropdown-submenu > a, .dropdown-submenu > .dropdown-toggle')
        .not(this)
...
// ===== CLOSE NAV ON WINDOW RESIZE (if going to desktop) =====
$(window).on('resize', function () {
  if ($(window).width() > 1025) {
    var $nav = $('.header-nav');
    // On desktop, remove 'none' class so nav is visible
    $nav.removeClass('none');
    $nav.find('.dropdown-menu').removeClass('toggle-menu');
    $('#mobileToggle').removeClass('active');
  } else {
    // On mobile, ensure nav is hidden if not toggled
    var $nav = $('.header-nav');
    if (!$nav.hasClass('none') && !$('#mobileToggle').hasClass('active')) {
      $nav.addClass('none');
    }
  }
});
...
document.querySelectorAll('#projectsDropdown').forEach(function(el) {
